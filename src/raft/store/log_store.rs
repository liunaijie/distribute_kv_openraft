...
impl LogStore {
    ...
    // persisted to disk
    fn flush_wal(&self) -> Result<(), io::Error> {
        self.db
            .flush_wal(true)
            .map_err(|e| io::Error::other(e.to_string()))
    }
...
impl RaftLogReader<TypeConfig> for LogStore {
    async fn try_get_log_entries<RB: RangeBounds<u64> + Clone + Debug + OptionalSend>(
        ...
        for item_res in it {
            ...
            let entry: EntryOf<TypeConfig> = serde_json::from_slice(&val).map_err(read_logs_err)?;
            res.push(entry);
        }
        Ok(res)
    }
...
impl RaftLogStorage<TypeConfig> for LogStore {
    ...
    async fn get_log_state(&mut self) -> Result<LogState<TypeConfig>, io::Error> {
        ...
        let last_log_id = match last {
            ...
            Some(res) => {
                ...
                let ent = serde_json::from_slice::<EntryOf<TypeConfig>>(&entry_bytes)
        ...
        I::IntoIter: OptionalSend,
    {
        ...
        let flush_res = self.flush_wal();
        callback.io_completed(flush_res);
