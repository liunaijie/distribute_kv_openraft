...
impl StateMachineStore {
    pub async fn new(
        db: Arc<DB>,
        snapshot_dir: PathBuf,
    ) -> Result<StateMachineStore, anyhow::Error> {
        ...
        sm.data.last_membership = sm.get_last_membership_()?;

        Ok(sm)
    }

    fn get_last_applied_(&self) -> Result<Option<LogId<TypeConfig>>, anyhow::Error> {
    ...
    fn get_last_membership_(&self) -> Result<StoredMembership<TypeConfig>, anyhow::Error> {
        let cf = self.cf_sm_meta();
    ...
    fn get_meta(
        &self,
    ) -> Result<(Option<LogId<TypeConfig>>, StoredMembership<TypeConfig>), anyhow::Error> {
        let cf = self.cf_sm_meta();
    ...
    pub fn get_value(&self, key: &String) -> Result<Option<Vec<u8>>, anyhow::Error> {
        let res = self.db.get_cf(self.cf_sm_data(), key);
        match res {
            Ok(e) => Ok(e),
